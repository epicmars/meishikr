...
import com.sin2pi.brick.components.gallery.ImageEntryLoader;
...
import static com.meishikr.app.view.activity.gallery.GalleryFragment.newInstance;

@BindLayout(R.layout.activity_gallery)
public class GalleryActivity extends BaseActivity<ActivityGalleryBinding> implements LoaderManager.LoaderCallbacks<Map<String, ImageFileEntry>>,
        ImageFileEntryViewHolder.OnImageFileEntryOperationListener, Handler.Callback, TakePhotoItemViewHolder.OnTakePhotoListener{
    ...
    public static void launchForResult(Activity context, int reqCode, int maxPhotoNum) {
        Intent intent = new Intent(context, GalleryActivity.class);
        intent.putExtra(Consts.Extras.MAX_PHOTO_NUM, maxPhotoNum);
        context.startActivityForResult(intent, reqCode);
    }

    public static void launch(Context context, int maxPhotoNum) {
        Intent intent = new Intent(context, GalleryActivity.class);
        intent.putExtra(Consts.Extras.MAX_PHOTO_NUM, maxPhotoNum);
    ...
    private void compressAndSave(ImageFileEntry entry) {
        File saveDir = context.getExternalFilesDir("photoPaths");
        if (!saveDir.exists()) {
            saveDir.mkdir();
        }
    ...
    public void onImageFileEntryClick(ImageFileEntry entry) {
        if(entry.isDirectory()) {
            ...
            galleryFragment = newInstance(entries);
            ft.replace(R.id.content, galleryFragment , TAG_GALLERY);
...
//            galleryFragment.updateImageFileEntries();
    ...
    private void setResultIntent() {
        ...
        if(!selectedImages.isEmpty()) {
            ...
            intent.putParcelableArrayListExtra(Consts.Extras.SELECTED_IMAGE_FILE_ENTRIES, entries);
            intent.putExtra(BlogEditFragment.EXTRAS_POST_UUID, getIntent().getStringExtra(BlogEditFragment.EXTRAS_POST_UUID));
            intent.putExtra(BlogEditFragment.EXTRAS_PARAGRAPH_INDEX, getIntent().getIntExtra(BlogEditFragment.EXTRAS_PARAGRAPH_INDEX, -1));
